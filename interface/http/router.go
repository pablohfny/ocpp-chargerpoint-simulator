...
func RegisterRoutes(
	...
	clientID, serverAddr string,
	connected *bool,
	reconnectFunc, disconnectFunc func() error,
) {
	// Initialize handlers
	statusHandler := handlers.NewStatusHandler(stationService, clientID, serverAddr, connected)
	actionHandler := handlers.NewActionHandler(stationService, reconnectFunc, disconnectFunc)
	configHandler := handlers.NewConfigHandler(stationService.GetConfigService())
	simulationHandler := handlers.NewSimulationHandler(stationService.GetSimulationService())
	logsHandler := handlers.NewLogsHandler(logService)
	ocppTriggerHandler := handlers.NewOCPPTriggerHandler(stationService)
